...
const Header = ({ hideGetStarted = false }: HeaderProps) => {
  ...
  const renderLinkClasses = (isActive: boolean) =>
    `text-sm sm:text-base md:text-lg font-semibold font-lato transition-colors duration-200 relative py-2 whitespace-nowrap ${
      isActive ? 'text-[#130f78]' : 'text-black hover:text-[#130f78]'
    }`;

  const renderMobileLinkClasses = (isActive: boolean) =>
    `text-base font-medium font-lato transition-all duration-200 py-3 border-b border-[#f0f0f0] last:border-b-0 hover:pl-2 hover:bg-[#f8f8f8] ${
      isActive ? 'text-[#130f78] font-semibold bg-[#f0f0f8]' : 'text-black hover:text-[#130f78]'
    }`;

  const isLinkActive = (path: string) => {
    if (path === '/about-us' || path === '/services') {
        ...
        <nav className="hidden sm:flex items-center gap-4 sm:gap-6 md:gap-8 lg:gap-10 xl:gap-12 flex-1 justify-center">
          {navLinks.map((link) => {
            ...
            if (link.path.startsWith('/#')) {
              return (
                <a key={link.name} href={link.path} className={renderLinkClasses(isActive)}>
                  {link.name}
                </a>
              );
            }
            ...
            return (
              <Link key={link.name} href={link.path} className={renderLinkClasses(isActive)}>
                {link.name}
              </Link>
            );
          })}
            ...
            if (link.path.startsWith('/#')) {
              return (
                <a
                  key={link.name}
                  href={link.path}
                  className={renderMobileLinkClasses(isActive)}
